# from https://github.com/sagepublishing/rejected_article_tracker_pkg/blob/main/rejected_article_tracker/src/ML/CrossRefUtils.py
import urllib
import requests
import time
import re
...
'''
The class below usually works with a config file. 
No config file here, so replaced with a dict
'''
try:
    MY_EMAIL = os.environ.get('MY_EMAIL','')
except:
    MY_EMAIL = ''
config = {'myemail':MY_EMAIL}
...
#                                             doi_batch_pool,
#                                             pool_size)
        for i,doi_batch in tqdm(enumerate(doi_batches),total = len(doi_batches)):
            cr_data_generator = self.get_doi_batch(doi_batch)
            for works_record in cr_data_generator:
#                 for works_record in works_record_batch:
                dois_found.add(works_record['DOI'])
                yield works_record
#             logger.debug('{} batches requested'.format(i))
...
#                 for works_record_batch in cr_data_generator:
            for doi in tqdm(remaining_dois):
                works_record_batch = self.get_one_crossref(doi)
